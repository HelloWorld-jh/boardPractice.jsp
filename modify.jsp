...
<%
	id = (String)session.getAttribute("id"); //login성공시 세션에 저장된 id값 가져오기
	
	String query = "select * from jmember where id = '" + id + "'";
...
%>
<!-- 위에서 얻은  db에 저장된 깂을 참조해서 수정하는 form을 만들어 수정후 서버에 보내어 db를 update -->
<form action="ModifyOk" method="post">
	이름 : <input type="text" name="name" size="10" value=<%=name %>><br />
	아이디 : <%=id %>	<input type="text" name="id" size="10" value=<%=id %> hidden><br />
